...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['comment'])) {
    ...
    if ($user_id && !empty($comment)) {
        $insert_comment_query = "INSERT INTO comments (post_id, user_id, content, status) 
                                 VALUES (?, ?, ?, 'pending')";
        $insert_stmt = $conn->prepare($insert_comment_query);
        $insert_stmt->bind_param("iis", $post_id, $user_id, $comment);
        $insert_stmt->execute();
        $insert_stmt->close();
        echo "<script>alert('Yorumunuz incelenmek üzere gönderildi.'); window.location.href = 'post.php?post_id=$post_id';</script>";
    }
}
...
<html lang="tr-TR"><head>
  ...
  <link rel="stylesheet" href="../css/profilephoto.css" media="screen">
        ...
        <div class="mb-5 border-top mt-4 pt-5">
          ...
          <?php while ($comment = $comment_result->fetch_assoc()) : ?>
            <div class="media mb-4">
              <img src="<?php echo $comment['profile_picture'] ? 'data:image/jpeg;base64,' . $comment['profile_picture'] : 'images/dprofile.jpg'; ?>" class="mr-3 rounded-circle" alt="">
              <div class="media-body">
                <h5><a href="profile.php?slug=<?php echo urlencode($comment['slug']); ?>" class="h4 d-inline-block mb-3">
